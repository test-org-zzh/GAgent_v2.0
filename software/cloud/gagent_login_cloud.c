...
int Cloud_Recive_M2minfo(char *server, int *port)
{
...
void GAgent_Login_Cloud(void)
{
    ...
    switch(g_ConCloud_Status)
    {
        case CONCLOUD_REQ_DID:
            ...
            if((GAgent_Mqtt_Register2Server(&g_stMQTTBroker)==0))
            {
                g_ConCloud_Status = CONCLOUD_REQ_DID_ACK;
                g_Xpg_GlobalVar.send2HttpLastTime = DRV_GAgent_GetTime_S();
                GAgent_Printf(GAGENT_INFO,"GAgent_Mqtt_Register2Server OK !");
                
            }
            else
            {
                GAgent_Printf(GAGENT_INFO,"GAgent_Mqtt_Register2Server Fail !");
            }
            break;
        case CONCLOUD_REQ_DID_ACK:
            ...
            ret = Cloud_Recive_Did(g_Xpg_GlobalVar.DID);
            if(ret == 0)
            {
        ...
        case CONCLOUD_REQ_M2MINFO_ACK:
            ...
            ret = Cloud_Recive_M2minfo(g_Xpg_GlobalVar.m2m_SERVER, &g_Xpg_GlobalVar.m2m_Port);
            if( ret==0 ) 
            {
        ...
        case CONCLOUD_REQ_LOGIN:
            ...
            g_Xpg_GlobalVar.connect2CloudLastTime = DRV_GAgent_GetTime_S();
            if((GAgent_Mqtt_Login2Server(&g_stMQTTBroker)==0))
            {
